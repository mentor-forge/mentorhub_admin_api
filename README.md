...
1. **Admin SPA Operator BFF (`/api/*`)**: Controlled CRUD for system Settings (`/api/setting`), Event publishing and querying (`/api/event`), and read-only audit log queries for external events (`/api/external-event`). Enforces `ROLE_ADMIN` RBAC and adheres to the OpenAPI 3.0.3 specification (`docs/openapi.yaml`). Pinned to `api-utils==1.0.0` (JSON-array list GETs, `offset`/`size` headers, no cursor envelope).
