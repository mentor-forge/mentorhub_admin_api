...
1. **Orchestrator** discovers all tasks, respects dependencies, and determines execution order.
   ...
   - Do not use **concurrent** agents to avoid testing conflicts.
2. **For each task**, the orchestrator Identifies the appropriate agent model. Most tasks should not require high reasoning, models. Do not use GROK models. Launch a new agent with:
   - The task file path
...
3. **Completion and documentation**
   ...
   - The sub agent does not need to commit changes, the orchestration agent handles that.
